# PM Studio — Architecture Overview

> "PM Studio architecture was designed to transform knowledge into reusable capabilities, enabling Product Managers to use Artificial Intelligence in a structured, scalable, and tool-independent way."

---

# Objective

PM Studio (Product Manager Operating System) is an open source AI Product Engineering platform.

Its purpose is to support Product Managers throughout the entire lifecycle of an initiative, organizing knowledge, executing workflows, and generating product artifacts consistently.

The project's goal is not just to integrate a Large Language Model (LLM), but to build a system capable of continuous evolution, maintaining low coupling, high cohesion, and a simple-to-understand architecture.

---

# Architecture Overview

PM Studio is composed of three main blocks:

1. User Interfaces
2. PM Studio Core
3. Workspace

Each block has well-defined responsibilities.

```text
                        User
                           │
                           ▼
               CLI • MCP • Web
                           │
                           ▼
──────────────────────────────────────────────────
                    PM Studio Core
──────────────────────────────────────────────────

        Workflows
             │
             ▼
     Initiative Repository
             │
             ▼
      Context Builder
             │
             ▼
      Prompt Builder
             │
             ▼
     AI Client (Contract)
             │
             ▼
 Infrastructure (Ollama, OpenAI...)
             │
             ▼
      Markdown Writer

──────────────────────────────────────────────────
                     Workspace
──────────────────────────────────────────────────

Signals and sources
Initiatives
Templates
Knowledge
Configuration
```

---

# Architectural Principles

Every architectural decision in PM Studio is guided by the following principles.

## Context before Prompt

Prompts are disposable.

Context is a permanent asset.

PM Studio invests in organizing and consolidating context before interacting with any AI model.

---

## AI is a Dependency

AI models are external services.

The PM Studio domain never depends directly on a specific implementation.

Today the platform supports Ollama, OpenAI-compatible providers, Anthropic,
and a local demo provider behind the same contract.

---

## Core First

Every business rule belongs to the PM Studio Core.

Interfaces, MCPs, or external applications never implement domain logic.

---

## Low Coupling

Components communicate through contracts (Protocols).

This allows replacing implementations without changing workflows.

---

## High Cohesion

Each component has only one responsibility.

This separation facilitates maintenance, testing, and evolution of the platform.

---

## Incremental Evolution

PM Studio evolves through small continuous deliveries.

Each Sprint seeks to strengthen the architecture before adding new capabilities.

---

# PM Studio Core Organization

The Core concentrates all the platform's intelligence.

Its current structure is organized as follows:

```text
src/
└── pm_os/
    ├── contracts/
    ├── domain/
    ├── infrastructure/
    ├── repositories/
    ├── workflows/
    ├── web/
    ├── writers/
    ├── templates/
    ├── bootstrap.py
    └── context_builder.py
```

Each directory has a specific responsibility.

| Component | Responsibility |
|------------|------------------|
| `contracts/` | Defines the contracts (Protocols) used by the application. |
| `domain/` | Contains the core concepts of the PM Studio domain. |
| `repositories/` | Retrieves information from the Workspace. |
| `workflows/` | Orchestrates the execution of platform capabilities. |
| `infrastructure/` | Concrete implementations of external services (Ollama, Logging, etc.). |
| `web/` | HTTP composition root, middleware, and application services used by the web interface. |
| `writers/` | Responsible for persisting generated artifacts. |
| `templates/` | Templates used by workflows. |

---

# The Domain

The central planning concept of PM Studio is the **Initiative**. A **Signal**
is reusable evidence that can precede and relate to one or more initiatives.
A **Decision** remains owned by its initiative specification while the
cross-initiative memory provides a derived, workspace-scoped view. This avoids
creating a second source of truth.

An Initiative represents a business problem or a product opportunity.

It concentrates all knowledge related to its lifecycle.

An Initiative can contain:

- discovery documents;
- meeting notes;
- research;
- requirements;
- PRDs;
- backlogs;
- roadmaps;
- RFCs;
- metrics;
- architectural decisions.

This decision was formalized in **ADR-004 — Initiative-Oriented Workspace**.

---

# Workspace

The Workspace represents the user's working area.

It contains all initiatives and artifacts produced by PM Studio.

Current structure:

```text
workspace/
├── signals/
│   ├── SIG-*.yaml
│   └── sources/
│       └── SSRC-*/
│           ├── metadata.yaml
│           └── source-file.pdf
└── initiatives/
    └── INT-0001-smart-supplier-query/
        ├── context/
        ├── artifacts/
        ├── logs/
        └── metadata.yaml
```

Signals and their uploaded sources carry the active personal or squad scope.
Uploaded sources are never converted into accepted evidence automatically:
the UI creates editable suggestions and persists only the signals confirmed by
the user.

## Context

Stores all raw knowledge used by workflows.

Examples:

- Discovery
- Interviews
- Meetings
- Notes
- Technical documentation

---

## Artifacts

Contains all documents generated by PM Studio.

Examples:

- PRD
- Backlog
- Roadmap
- RFC
- Executive Summary

---

## Metadata

Represents the Initiative's identity.

In the future it will store information such as:

- identifier;
- owner;
- status;
- tags;
- executed workflows;
- generated artifacts.

---

## Logs

Stores the execution history of workflows related to the Initiative.

---

# Execution Flow

The main PM Studio workflow follows the sequence below:

```text
User
    │
    ▼
Interface
    │
    ▼
Workflow
    │
    ▼
Initiative Repository
    │
    ▼
Context Builder
    │
    ▼
Prompt Builder
    │
    ▼
AI Client (Contract)
    │
    ▼
Infrastructure
(Ollama, OpenAI...)
    │
    ▼
Markdown Writer
    │
    ▼
Artifact
```

Each component has a single responsibility and can evolve independently.

---

# Capabilities

PM Studio is organized around reusable capabilities.

Each capability represents a complete product workflow.

Planned examples:

- Create PRD
- Create Backlog
- Create Roadmap
- Create RFC
- Executive Summary
- AI Review
- Security Review

All reuse the same Core architecture.

---

# Architecture Benefits

The architecture was designed to provide:

- incremental evolution;
- low coupling;
- high cohesion;
- ease of testing;
- component reusability;
- independence between domain and infrastructure;
- support for multiple AI providers;
- ease of open source community contribution.

---

# Current Boundaries and Next Steps

The architecture will continue evolving as new capabilities are added.

The current architecture intentionally targets a single PM Studio installation.
Background jobs use a bounded in-process executor and persist progress in SQLite.
This keeps local and self-hosted operation simple, but a multi-instance deployment
would require a shared queue and database.

The next architectural topics are:

- split the remaining HTTP composition root into route modules;
- add structured telemetry and latency budgets;
- introduce a shared job queue only when multi-instance deployment is required;
- continue source and citation quality measurement;
- evaluate a Vector Store from a demonstrated retrieval need.

---

# Summary

PM Studio is not just a document generator.

It is an AI Product Engineering platform built to transform knowledge into reusable capabilities.

Its architecture was designed to reflect the mental model of Product Managers, maintaining a clear separation between domain, infrastructure, and interfaces.

This approach allows the project to evolve incrementally, maintaining simplicity, scalability, and ease of collaboration.
